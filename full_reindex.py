#!/usr/bin/env python3
"""Полная переиндексация базы МДМ через OpenAI API"""
import json
import os
import sys
...
print("Полная переиндексация МДМ через OpenAI", file=sys.stderr)
print("=" * 50, file=sys.stderr)
...
print(f"✓ Загружено {len(records)} записей из JSONL", file=sys.stderr)
